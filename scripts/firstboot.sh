        ...
        nfs)
            ...
            if mount -t nfs "$NFS_SERVER:$NFS_EXPORT" "$mount_point" -o ro,soft,timeo=50,_netdev; then
                ...
                if ! grep -qF "$NFS_SERVER:$NFS_EXPORT" /etc/fstab; then
                    echo "$NFS_SERVER:$NFS_EXPORT $mount_point nfs ro,soft,timeo=50,_netdev,nofail,x-systemd.before=docker.service,x-systemd.wanted-by=docker.service 0 0" >> /etc/fstab
                fi
        ...
        smb)
            ...
            if timeout 60 mount -t cifs "//$SMB_SERVER/$SMB_SHARE" "$mount_point" -o "$mount_opts"; then
                ...
                if ! grep -qF "//$SMB_SERVER/$SMB_SHARE" /etc/fstab; then
                    echo "//$SMB_SERVER/$SMB_SHARE $mount_point cifs ${mount_opts},nofail,x-systemd.before=docker.service,x-systemd.wanted-by=docker.service 0 0" >> /etc/fstab
                fi
                export MUSIC_PATH="$mount_point"
