...
snapMULTI è pensato per chi vuole un **sistema audio multi-room open source** senza costruirsi a mano tutto lo stack Linux audio. Devi comunque flashare Raspberry Pi OS, scaricare il software (uno ZIP o se sei capace usare git da linea di comando) e rispondere a poche domande sulla tua configurazione; snapMULTI automatizza la parte difficile — Snapcast, Docker, routing audio, discovery dei servizi (mDNS), boot read-only e diagnostica di recupero. Fai cast da **Spotify**, **AirPlay**, **Tidal** o dalla tua libreria; tutti gli altoparlanti suonano in modo sincronizzato. I servizi di streaming mantengono i loro requisiti di account.
