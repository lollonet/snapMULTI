...
- Install takes ~10 minutes on Pi 4 — reflash is cheap (this ADR was written against the v0.7-era install footprint; current user-facing docs document ~15-20 minutes on Pi 4/5 — longer on Pi 3 or Pi Zero 2 W — see [INSTALL.md](../INSTALL.md). The rationale still holds: the install is short enough to treat as the update mechanism.)
...
Architectural changes are gated by `device-smoke.sh --both` on a real Pi, not just CI. LOC reduction is a secondary metric. The primary metric is: pi-server reboots green without manual intervention.
