...
> **Eccezioni — stato che sopravvive alla pulizia overlayroot:** `snapmulti-state-backup.path` vigila i cambiamenti dello stato myMPD (`/opt/snapmulti/mympd/workdir/state` — la sottodirectory change-frequent in cui myMPD scrive quando l'utente tocca playlist / impostazioni / script). Quando la `.path` unit si attiva, lo script di backup fa snapshot dell'INTERA workdir myMPD su `/boot/firmware/snapmulti-backup/` — state + config + smartpls + scripts + pics — non solo la sottodirectory `state/` da cui è stata triggerata, così playlist intelligenti, temi e script personalizzati dall'utente sopravvivono a un reflash. `snapmulti-state-backup.timer` è la rete di sicurezza: gira ogni 5 min e cattura `server.json` di snapserver (l'event-driven su quel file non è praticabile — snapserver lo riscrive ogni ~3 s per rinfrescare gli heartbeat `lastSeen` dei client). `backup-snapmulti-state.sh` si interrompe su uguaglianza canonica (ordinamento chiavi + strip `lastSeen` + sha256) così i tick no-op costano zero scritture FAT32. `snapmulti-backup.timer` salva separatamente MPD `mpd.db` al boot e su cadenza giornaliera. Ad ogni boot, `restore-snapmulti-state` (wired come `ExecStartPre` su `snapmulti-server.service`) ricopia il backup in `/opt/snapmulti/` prima che i container partano. L'approccio bind-mount di `snapmulti-data-persistence.service` di v0.7.9 è stato rimosso in v0.7.9.1 (vedi #527) — era un no-op su overlayroot in modalità tmpfs.
