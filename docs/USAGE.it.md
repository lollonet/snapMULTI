...
- Server: `snapmulti-server.service`, `snapmulti-data-persistence.service` (bind-mount overlayroot per stato snapserver + myMPD — vedi [ADVANCED.it.md](ADVANCED.it.md#filesystem-read-only)), `snapmulti-status.timer`, `snapmulti-backup.timer`
