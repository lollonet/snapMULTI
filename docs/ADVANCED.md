...
> **Exceptions — state that survives overlayroot wipes:** `snapmulti-state-backup.path` watches the whole myMPD workdir (`/opt/snapmulti/mympd/workdir`) and snapshots it to `/boot/firmware/snapmulti-backup/` on change (event-driven). Scope is the WHOLE workdir — state + config + smartpls + scripts + pics — not just the `state/` subdir, so user-customised smart playlists / themes / scripts survive a reflash. `snapmulti-state-backup.timer` is the safety net: it runs every 5 min and captures snapserver `server.json` (event-driven on that file is not viable — snapserver rewrites it every ~3 s to refresh client `lastSeen` heartbeats). `backup-snapmulti-state.sh` short-circuits on canonical equality (sort keys + strip `lastSeen` + sha256) so no-op ticks cost zero FAT32 writes. `snapmulti-backup.timer` separately backs up MPD `mpd.db` on boot/daily cadence. On every boot, `restore-snapmulti-state` (wired as `ExecStartPre` on `snapmulti-server.service`) copies the state backup back into `/opt/snapmulti/` before containers start. v0.7.9's `snapmulti-data-persistence.service` bind-mount approach was removed in v0.7.9.1 (see #527) — it was a no-op on tmpfs-mode overlayroot.
