...
#
# Discovery: snapclient relies on its built-in libavahi-client mDNS
# browse (no --host pinned) — auto-recovers when the snapserver
# changes IP. The dual-stack mDNS ambiguity that previously made
# snapclient latch onto IPv6 link-local SRV targets is killed at
# kernel level via `ipv6.disable=1` in cmdline.txt (ADR-007 + the
# prepare-sd.sh tweak). PR #521's ExecStartPre/--host workaround
# was retired by that decision.
