...
detect_music_library() {
    ...
    info "Scanning for music libraries..."
    ...
    for pattern in "${search_paths[@]}"; do
        ...
        for dir in $pattern; do
            ...
            if [[ "$count" -gt 0 ]]; then
                info "  Found: $dir ($count audio files)"
                candidates+=("$dir:$count")
...
if [[ -f .env ]]; then
    ...
    # Auto-detect music library
    MUSIC_DETECTED="$(detect_music_library)"
    if [[ -n "$MUSIC_DETECTED" ]]; then
        MUSIC_PATH="$MUSIC_DETECTED"
...
# Music library path (auto-detected or default)
