...
### Fixed
- **Pi Zero 2W native install switched from badaix `.deb` to distro apt** — Pi OS Trixie (Debian 13, current Raspberry Pi Imager default) ships `libflac13` while the badaix `snapclient_0.35.0-1_*_bookworm.deb` declares `libflac12 (>= 1.3.0)` as a hard dependency. The install died on `E: Unable to locate package libflac12` during dependency pre-fetch. Reverted to installing `snapclient` straight from the distro apt repo: Trixie ships v0.31.0, Bookworm ships v0.27.0. The Snapcast wire protocol is stable across these minor versions so the server's v0.35 builds keep talking to either client. Removed SHA256 pinning + manual `.deb` download / verification path along with the bookworm-only assumption.
