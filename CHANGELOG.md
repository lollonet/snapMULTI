...
### Fixed
- **NFS / SMB music library invisible after first boot under overlayroot** — overlayroot's initramfs script rewrites `_netdev` fstab entries even when `overlayroot.local.conf` says `recurse=0`, leaving the NFS / SMB mount visible only at `/media/root-ro/media/<src>-music` (the lower-layer path) while the natural `/media/<src>-music` ends up empty. MPD bind-mounts the empty path and serves an empty library. Reproduced live on snapvideo today after a clean v0.6.5 reflash. Added `scripts/common/snapmulti-music-bind.{service,sh}` — a oneshot systemd unit ordered `Before=docker.service snapmulti-server.service` that bind-mounts the lower-layer view back to the natural path. Idempotent (`mountpoint -q` guard), no-op when overlayroot is inactive (`ConditionPathIsMountPoint=/media/root-ro`), only installed by `firstboot.sh` for server / both modes with `MUSIC_SOURCE` in `nfs|smb`. Workaround until upstream overlayroot honours `recurse=0` for `_netdev` entries
- **`firstboot.sh` headless detection ignored DSI / DPI / DP / eDP panels** — the local `has_display()` only scanned `card*-HDMI-*/status`, so a Pi with the official 7" Touch Display (DSI) was forced into headless mode regardless of what `client/common/scripts/display.sh` (the canonical source-of-truth, supports HDMI/DSI/DPI/DP/eDP since PR #305) would have reported. `firstboot.sh` now sources `display.sh` from the SD-card staging path, falling back to the conservative HDMI-only function only if the file is missing
- **`setup.sh --auto --no-readonly <config>` silently dropped the config path** — the parser only captured `AUTO_CONFIG` when it followed `--auto` *immediately* AND was not a flag. `firstboot.sh` calls setup.sh in exactly that broken shape (`--auto --no-readonly /boot/firmware/install.conf`), so `install.conf` was never sourced, losing `DISPLAY_MODE` and `SNAPSERVER_HOST=127.0.0.1` in `--both` mode (snapclient fell back to mDNS discovery instead of binding directly to localhost). New parser accepts the positional config independently of flag order
- **CLI `--no-readonly` could be silently undone by the config file** — the override was applied BEFORE sourcing `AUTO_CONFIG`, and the default `snapclient.conf` ships with `ENABLE_READONLY=true`, so the source flipped readonly back on. Now the parser records `CLI_READONLY_OVERRIDE` separately and applies it AFTER the source, giving precedence: explicit CLI > config > default
