...
### Fixed
- **Smoke mDNS section over-counted AirPlay / Spotify by including non-snapMULTI receivers** — `device-smoke.sh:_check_mdns_service()` counted all `^=` resolved entries from `avahi-browse -rpt _raop._tcp` and `_spotify-connect._tcp`. Those protocols are not snapMULTI-exclusive: macOS "AirPlay Receiver" (port 7000), Apple TVs, HomePods, AVRs, Echos, Sonos, smart TVs all publish them. Result on mrbike 2026-05-11: `[OK] AirPlay mDNS fully resolves (7 entry/entries)` while Snapcast and Spotify showed 6 each — the +1 was a Mac Studio's macOS AirPlay receiver (`jupiter.local:7000`), not a snapMULTI publishing fault. Same blind spot for Spotify: a Sonos / Echo on the LAN would inflate the count and mask a real snapMULTI Spotify Connect outage. Fix: enumerate snapMULTI hostnames via `_snapcast._tcp` (our anchor — only we publish this), then filter AirPlay / Spotify browse output to entries whose mDNS hostname (field 7 of avahi-browse `-rpt` output) is in that list. Post-fix on mrbike: Snapcast=6 / AirPlay=6 / Spotify=6 — symmetric (3 snapMULTI servers × IPv4+IPv6), with the `(snapMULTI only)` suffix on the filtered protocols so operators know the count is post-filter
...
- **`deploy.sh` now writes `MUSIC_SOURCE` to `.env`** — previously `.env` carried only `MUSIC_PATH`. The `device-smoke.sh` Tier-2 music-library check (added in v0.6.4 via PR #303) gated on `MUSIC_SOURCE=nfs|smb` and therefore NEVER triggered on real installs (verified live on snapvideo: `MUSIC_SOURCE='unset'` despite the device using NFS). Source value is taken from the `MUSIC_SOURCE` env variable when `mount-music.sh` set it (nfs / smb / usb / streaming-only), or inferred from the mount point otherwise (network / usb / local). Smoke check accepts `nfs|smb|network` as "must validate content"
...
- **`device-smoke.sh`: five new health checks** — distilled from the live troubleshooting on 2026-05-07 where each of these conditions had a real-world manifestation in the same hour:
  ...
  - **IP conflict detection** — `arping -D` duplicate-address probe. Catches scenarios like the F8:17:2D PC HP statically configured with `.95` while snapvideo also held `.95` via DHCP — the kind of intermittent packet-loss bug that's invisible in steady state
