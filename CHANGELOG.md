...
- **`device-smoke.sh`: seven Tier-2 health checks** — round of follow-ups to the Tier-1 work, each motivated by a class of issues we want surfaced at install time (instead of weeks later as a "things suddenly stopped working" support ticket):
  ...
  - **HAT consistency** (client/both) — `SOUNDCARD=hw:CARD=NAME,DEV=0` in the client `.env` must reference a `CARD=NAME` that actually appears in `aplay -l`. Catches stale or mis-detected HAT configs (e.g. user swapped from HiFiBerry to InnoMaker without re-running setup.sh)
